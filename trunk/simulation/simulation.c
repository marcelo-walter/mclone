...
void initPatFormAndGrowth( RELAXMODE rMode )
{
	...
	//TODO, se criar células antes de mudar o tamanho tem que ser aqui
	...
	while( time < finalTime )
	{
		...
		if(rMode == LOCALRELAX)
		{
			...
			if(!findElement(&listFaces, wFace))
			{
				...
				i += 1;
				listFaces.array = realloc(listFaces.array,sizeof(int)*(i+1));
			}	listFaces.size = i;
