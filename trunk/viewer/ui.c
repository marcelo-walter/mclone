...
//extern void outputViewWindowAsRGB (const char *file_name);
extern int loadSessionFile( char *inFile );
/* declared in voronoi.c */
extern void computeVoronoi( void );
extern void patFormAndGrowthSimulation( void );
extern void initPatFormAndGrowth( void );
/* declared in forces.c */
extern void planarizeFace( int whichFace );

int WithInterpolationType; //USed to set the type os RBF Vector Field Interpolation we will use.
...
/* declared in primitives.c */
extern int activePrim;
extern int activeLandMark;

/* viewing mode */
extern VMODE viewMode;

extern float orth[3][6];
extern flag mouseMovingFlag;	/* this is only to display
 wireframe when the user
 is manipulating the trackball */
...
{
    int rg, cg, ra, ca;
    int width, height, pixelDepth, refreshRate;
